...
Four time-separated full-canvas captures demonstrate Ari and Sol changing frames. Mina's alternate working frame is demonstrated separately by two captures of her same 48 × 64 CSS-pixel clearance region, recorded 83 ms apart with no selected agent or selection indicator. The offscreen Electron backing images are both 96 × 128 pixels. SHA-256 is calculated over exactly 36,864 decoded pixel bytes per image: 8-bit interleaved RGB, no alpha or row padding, ordered left-to-right within each row and top-to-bottom across rows. Mina frame A hashes to `14b3dacf75496158d57c2579d4efb2a1342fb23e658f00fa3b5ff5ffbb390934`; Mina frame B hashes to `8ea4389ff4caab28993dab715c8646f90e0f1f57317ceebf9b5429c711e8b27e`. Direct decoded-pixel comparison found 512 changed pixels, confined to backing-pixel bounds x=20–75 and y=59–82 around Mina's arm and tablet pose; the room background and crop remained unchanged. These still images establish distinct rendered frames, not animation looping; looping was observed during the prior runtime verification and is separately supported by the tested `repeat: -1` configuration.

Reproduce the decoded RGB hashes with Pillow using `python3 -c 'import hashlib,sys; from PIL import Image; [print(path, hashlib.sha256(Image.open(path).convert("RGB").tobytes()).hexdigest()) for path in sys.argv[1:]]' docs/verification/assets/mina-animation-frame-a.png docs/verification/assets/mina-animation-frame-b.png`.
